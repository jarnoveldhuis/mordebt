    ...
    <div className={`text-sm ${className}`}>
      ...
      {showDetails && (
        <div className="mt-2 text-xs text-gray-600 bg-gray-50 p-2 rounded border border-gray-200">
          ...
          <div>
            <strong>Connected Since:</strong> {localStorage.getItem('plaid_access_token_info') ? 
              new Date(JSON.parse(localStorage.getItem('plaid_access_token_info') || '{}').timestamp || 0).toLocaleString() :
              'Unknown'
            }
          </div>
          <div className="mt-1">
